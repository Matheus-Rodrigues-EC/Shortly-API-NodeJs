...
export default function signUpValidation(req, res, next){
    ...
    if(signUpSchema.validate({Name: name, Email: email, Password: password}).error !== undefined){
        ...
        if(signUpSchema.validate({Name: name, Email: email, Password: password}).error.message === '"Email" is required'){
        ...
        if(signUpSchema.validate({Name: name, Email: email, Password: password}).error.message === '"Password" is required'){
